...
/**
 * Client-side registry of {@link EntityAgentLoop} instances — one per Tulpa
 ...
 * back to the same loop via {@link ClientTulpaLookup}. The int id is only ever
 * used as an ephemeral handle for the current tick.
 *
 * <h2>Single-layer architecture</h2>
 * Each Tulpa carries its own conversation; the owner chats with each entity
