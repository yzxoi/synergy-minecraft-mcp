package com.dwinovo.tulpa.pathing.viz;
...
public final class PathVizPublisher {
     ...
     */
    public static void publish(TulpaPlayer player, Path path, List<BlockPos> targets) {
     ...
     * {@code REQUEST_PAUSE}.
     */
    public static void publishTargets(TulpaPlayer player, List<BlockPos> targets) {
        ServerPlayer owner = player.resolveOwnerPlayer();
    ...
    public static void clear(TulpaPlayer player) {
        ServerPlayer owner = player.resolveOwnerPlayer();
        if (owner == null) return;
        Services.NETWORK.sendToPlayer(owner, new PathVizPayload(
                player.getUUID(), player.level().dimension().identifier(),
