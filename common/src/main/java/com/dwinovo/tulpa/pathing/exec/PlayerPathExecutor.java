...
import com.dwinovo.tulpa.pathing.movement.Movement;
import com.dwinovo.tulpa.pathing.movement.Moves;
...
/**
 * Walks a computed {@link Path} on a companion {@link TulpaPlayer} body. The
...
public final class PlayerPathExecutor {
    ...
    public PlayerPathExecutor(TulpaPlayer player, Path path, double speed,
    ...
    private Status replan(String why) {
        ...
        Constants.LOG.info("[tulpa-path] REPLAN {} | {}", why, mv != null ? desc(mv) : "feet=" + feet().toShortString());
    ...
    private Status trackProgress(Movement mv) {
        ...
        if (ticksSinceProgress >= STUCK_WARN_TICKS && !stuckWarned) {
            stuckWarned = true;
            Constants.LOG.warn("[tulpa-path] STUCK no forward progress {} ticks | {}",
