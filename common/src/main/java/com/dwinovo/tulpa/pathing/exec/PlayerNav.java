...
import com.dwinovo.tulpa.pathing.calc.AStar;
import com.dwinovo.tulpa.pathing.calc.AStarSearch;
import com.dwinovo.tulpa.pathing.calc.NavContext;
import com.dwinovo.tulpa.pathing.calc.NavGoal;
import com.dwinovo.tulpa.pathing.calc.Path;
import com.dwinovo.tulpa.pathing.util.BlockHelper;
import com.dwinovo.tulpa.pathing.viz.PathVizPublisher;
...
/**
 * Plan → execute → replan driver for a companion {@link TulpaPlayer} body.
 ...
 * cell ({@link #PlayerNav(TulpaPlayer, BlockPos, double, BooleanSupplier)}) or
...
public final class PlayerNav {
    ...
    public PlayerNav(TulpaPlayer player, BlockPos goal, double speed, BooleanSupplier reached) {
        this(player, () -> resolveBlockGoal(player, goal), speed, reached, true);
    }

    /** Walk to a (possibly moving) single cell. */
    public PlayerNav(TulpaPlayer player, Supplier<BlockPos> goalSupplier, double speed,
    ...
    public static PlayerNav toGoal(TulpaPlayer player, Supplier<NavGoal> goalSupplier,
                                   double speed, BooleanSupplier reached) {
        return new PlayerNav(player, goalSupplier, speed, reached, true);
    }

    private PlayerNav(TulpaPlayer player, Supplier<NavGoal> goalSupplier, double speed,
    ...
    private static NavGoal resolveBlockGoal(TulpaPlayer player, BlockPos bp) {
    ...
    private NavContext searchContext() {
        if (player.level() instanceof net.minecraft.server.level.ServerLevel sl) {
            com.dwinovo.tulpa.pathing.cache.PathCaches.ensureSnapshot(sl, player.blockPosition());
        }
    ...
    private static java.util.concurrent.CompletableFuture<Path> runAsync(AStarSearch s) {
        return com.dwinovo.tulpa.pathing.calc.PathPlannerPool.submit(() -> runToCompletion(s));
    }
    ...
    private static Path runToCompletion(AStarSearch s) {
        try {
            ...
            com.dwinovo.tulpa.Constants.LOG.error("path search failed", t);
            return null;
        }
    }
    ...
    private static it.unimi.dsi.fastutil.longs.LongSet pathHashes(Path p) {
        ...
        for (com.dwinovo.tulpa.pathing.movement.Movement m : p.movements) {
