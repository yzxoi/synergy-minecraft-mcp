...
你：    挖一组铁矿回来
Numen： 这就去。下矿找铁。
        ▸ 4 步 · locate_biome · goto · mine · collect_items   ✔
...
2. **创建模型配置。** 按 **`N`** → **设置 → 模型配置**，新建配置并填入 provider、模型和你自己的 API key。
3. **召唤一个同伴。** 点面板左栏的 **`+`**，给它起个名字，回车。
4. **点它的头像开聊**，把要做的事告诉它。剩下的，交给它。

> 按 `N` 的面板有三页：**Chat**（聊天 + 实时计划面板）、**Items**（一张仿原版背包的只读角色卡）、**Settings**（填 key 和模型）。左栏就是同伴名册——点头像切换、点 **`+`** 召唤、点 **`✕`** 注销，基本不用敲指令。左边缘还有个小头像 HUD——它说话时，头像和气泡会一起滑出来。

1.21.11 新版的安装、Synergy 外接大脑、异步任务、蓝图与排障说明见[详细使用手册](docs/USER_GUIDE_1.21.11_CN.md)。
...
- ⚔️ **真战斗**——具身战术控制器持续观察多个威胁，确定性地选择追击、侧移、攻击、拉扯或撤退，并在驱动原生玩家战斗前执行生命与环境安全检查。
...
36 个这样的原生工具，拼成它此刻的双手与双眼。而它的本事还能继续往上长——靠的正是[通达](#通达)的那两件法宝：
